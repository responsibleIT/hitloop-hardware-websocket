class InkColor extends Scene {
    constructor(deviceManager) {
      ...
      this._configuredDevices = new Set();
    ...
    draw() {
      ...
      for (const dev of devices) {
        ...
        // Use full sensor data for motion (supports d1-d4)
      ...
      for (let i = this.particles.length - 1; i >= 0; i--) {
        ...
        // Use stored color
        const c = p.col || [0, 0, 0];
        this.inkLayer.fill(c[0], c[1], c[2], Math.min(255, Math.abs(T)));
    ...
    _getTilt(data) {
      // 1. Try raw accelerometer (handheld)
      // Standard mapping: 0..255 => -2g..+2g
      if (data.ax !== undefined || data.aX !== undefined || data.accX !== undefined) {
          ...
          let ax, ay;
          // Check if values are likely 0-255 (centered at 128)
          // or -2..2 (already normalized)
      ...
      // 2. Fallback to balance board dNW/etc
    ...
    _getColor(state, id, tilt) {
      ...
      // Deadzone check to avoid flickering at rest? 
      // User said "based on tilting". 
      // If no tilt, keep last target? Or default?
      // Let's use simple quadrants.
