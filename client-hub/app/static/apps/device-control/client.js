...
function getDevicePosition(device) {
  ...
  return {
    x: constrain(x, 25, canvasWidth - 25),
    y: constrain(y, 25, canvasHeight - 25),
  };
}
...
function setupIndividualLeds() {
  const container = document.getElementById("individualLedsContainer");
  ...
  for (let i = 0; i < 6; i++) {
    ...
    colorInput.id = `ledColor${i}`;
    colorInput.value = "#ff0000";

    const buttonsDiv = document.createElement("div");
    buttonsDiv.className = "led-control-buttons";
    ...
    setBtn.addEventListener("click", () => {
      ...
      selectedDevices.forEach((deviceId) => {
        manager.sendCommandToDevice(deviceId, "led_set", i, color);
      });
    });
    ...
    offBtn.addEventListener("click", () => {
      selectedDevices.forEach((deviceId) => {
        manager.sendCommandToDevice(deviceId, "led_off", i);
      });
    });
    ...
    ledControl.appendChild(label);
    ledControl.appendChild(colorInput);
    ledControl.appendChild(buttonsDiv);

    ledGrid.appendChild(ledControl);
  }
...
function setupCommandHandlers() {
  ...
  // LED Brightness
  document
    .getElementById("applyLedBrightness")
  ...
  // Turn all LEDs off
  document.getElementById("applyAllLedsOff").addEventListener("click", () => {
    selectedDevices.forEach((deviceId) => {
      manager.sendCommandToDevice(deviceId, "led_all_off");
    });
  });
}
...
function draw() {
  ...
  if (!manager || !manager.ws || manager.ws.readyState !== WebSocket.OPEN) {
    ...
    textAlign(LEFT, TOP);
    text(
      "Connect to WebSocket to see devices. Drag to select devices for control.",
      20,
      20,
    );
