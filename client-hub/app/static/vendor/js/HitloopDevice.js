...
class HitloopDevice {
  ...
  parseHexData(hexString) {
    ...
    try {
      ...
      if (
        [ax, ay, az, dNW, dNE, dSE, dSW, tapValue].some(
          (n) => !Number.isFinite(n),
        )
      ) {
        return false;
  ...
  validateCommand(command, params) {
    ...
    if (actualParamCount !== expectedParamCount) {
      console.warn(
        `Command '${command}' expects ${expectedParamCount} parameters, got ${actualParamCount}`,
      );
      return false;
    }
    ...
    switch (command) {
      case "brightness":
        ...
        if (isNaN(brightness) || brightness < 0 || brightness > 255) {
          console.warn(
            `Invalid brightness value: ${params[0]}. Must be between 0 and 255.`,
          );
          return false;
        }
        break;
      case "spring_param":
        if (!/^[0-9a-fA-F]{6}$/.test(params[0])) {
          console.warn(
            `Invalid spring parameters: ${params[0]}. Must be 6 hex characters.`,
          );
          return false;
        }
        break;
      case "vibrate":
        ...
        if (isNaN(duration) || duration < 0) {
          console.warn(
            `Invalid vibration duration: ${params[0]}. Must be positive.`,
          );
          return false;
        }
        break;
  ...
  sendCommand(command, ...params) {
    if (!this.ws || this.ws.readyState !== WebSocket.OPEN) {
      console.warn(
        `Cannot send command to device ${this.id}: WebSocket not connected`,
      );
      return false;
    }
  ...
  /**
   * Set individual LED on with color (independent control)
   * @param {number} ledIndex - LED index (0-5)
   * @param {string} color - Color (hex or named)
   * @returns {boolean} True if command was sent successfully
   */
  setLedOn(ledIndex, color) {
    if (ledIndex < 0 || ledIndex > 5) {
      console.error(`LED index out of range: ${ledIndex}`);
      return false;
    }
    const hexColor = this.parseColor(color);
    const hexString = this._rgbToHex(hexColor);
    return this.sendCommand("led_set", `${ledIndex}:${hexString}`);
  }
  ...
  setLedOff(ledIndex) {
    ...
    return this.sendCommand("led_off", ledIndex.toString());
  }

  /**
   * Turn off all LEDs (individual mode)
   * @returns {boolean} True if command was sent successfully
   */
  setAllLedsOff() {
    return this.sendCommand("led_all_off");
  }

  /**
   * Set multiple LEDs at once
   * @param {Array<{index: number, color: string}>} leds - Array of {index, color} objects
   * @returns {boolean} True if all commands were sent successfully
   */
  setMultipleLeds(leds) {
    let allSuccess = true;
    for (const led of leds) {
      if (!this.setLedOn(led.index, led.color)) {
        allSuccess = false;
      }
    }
    return allSuccess;
  ...
  /**
   * Get state of all LEDs (debug)
   * @returns {boolean} True if command was sent successfully
   */
  getLedState() {
    return this.sendCommand("led_get_state");
  }

  /**
   * Helper method to convert RGB array to hex string
   * @private
